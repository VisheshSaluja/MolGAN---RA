...
score_dir = os.path.join(os.path.dirname(__file__), 'data')

with open(os.path.join(score_dir, 'NP_score.pkl'), 'rb') as f:
    NP_model = pickle.load(f)

with open(os.path.join(score_dir, 'SA_score.pkl'), 'rb') as f:
    raw_sa_model = pickle.load(f)
    SA_model = {i[j]: float(i[0]) for i in raw_sa_model for j in range(1, len(i))}
